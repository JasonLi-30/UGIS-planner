...
#include <UGIS_motion_planner/UGIS_planner.h>
#include <UGIS_motion_planner/UGIS_utils.h>
...
{
static const rclcpp::Logger LOGGER = rclcpp::get_logger("UGIS planner");
UGIS::UGIS(UGIS_Trajectory* trajectory, planning_scene::PlanningScenePtr& planning_scene,
                  const std::string& planning_group, moveit::core::RobotState& start_state, UGIS_Parameters params, Ort::Session* session_presample)
...
bool UGIS::planning(double& length)   //路径规划算法主干内容
{
  ...
  bool result = UGISToTrajectory(TEMP_Path, length);
  ...
  if(do_export_dataset_)
  {
    ...
    saveToBinary(dataset_, "UGIS_training_data.bin");
    std::cout << "===UGIS_training_data.bin has been exported===" << std::endl;
  }
...
int UGIS::RRTConnectSteerNode(RRT_Node* goal, std::vector<RRT_Node>& Tree, double maxDis, bool flag_CCD)
{
  RRT_Node start, temp;
...
double UGIS::SED(RRT_Node* Node1, RRT_Node* Node2)
{
  double output = 0;
...
double UGIS::FindNearestNodes(std::vector<RRT_Node>& Tree1, std::vector<RRT_Node>& Tree2, int& iTree1Node, int& iTree2Node, bool flag_CCD, double threshold)
{
...
bool UGIS::FindParentNode(RRT_Node* node, std::vector<RRT_Node>& Tree, bool flag_CCD)
{
  double min_dist = INFINITY;  
  double dist;
  bool output = false;
...
RRT_Node UGIS::RLPresampleNode(RRT_Node& parent, RRT_Node& goal)
{
  RRT_Node output;
  output.node.in_Coli = false;

  int rl_joints = model_joints_;
  if (rl_joints <= 0 || rl_joints > num_joints_)
    rl_joints = num_joints_;
  int extra_joints = num_joints_ - rl_joints;
  ...
  std::vector<float> parent_q(parent_config.begin() + extra_joints, parent_config.end());
  std::vector<float> goal_q(goal_config.begin() + extra_joints, goal_config.end());
  auto output_names = session_presample_->GetOutputNames();

  std::vector<int64_t> parent_q_shape = {1, rl_joints};
  std::vector<int64_t> goal_q_shape   = {1, rl_joints};
  ...
  for (int i = 0; i < extra_joints; ++i)
    output.node.configuration[i] = parent_config[i];
  for (int i = 0; i < rl_joints; ++i)
    output.node.configuration[extra_joints + i] = output_data[i];

  return output;
}


RRT_Node UGIS::RRTConnectGenNode(std::vector<RRT_Node>& parent, double* RND_Mean_, double* STD_dev, RRT_Node* goal, double maxDis, bool flag_CCD)
{
...
Eigen::MatrixXd UGIS::PathSplicing(Eigen::MatrixXd Path1, std::vector<RRT_Node> Path2, int iStart, int iEnd)
{
...
bool UGIS::getPointColi(std::vector<double>& joint_states)
{
...
void UGIS::printObstacleInfo(const planning_scene::PlanningScenePtr& scene)
{
...
bool UGIS::getCCD(RRT_Node* Node1, RRT_Node* Node2, double Stepsize, bool mode)
{
...
int UGIS::UGISOnce(std::vector<RRT_Node>& output, RRT_Node* LocalStart, RRT_Node* LocalEnd, double maxDis, bool flag_CCD, bool init)
{
...
void UGIS::Simplify(std::vector<RRT_Node>& input, std::vector<RRT_Node>& output, double maxDis, int flag_CCD, double length_total)
{
  // 路径简化
...
void UGIS::Simplify_longest(std::vector<RRT_Node>& input, std::vector<RRT_Node>& output, double maxDis)
{
...
bool UGIS::UGISToTrajectory(std::vector<RRT_Node>& output, double& length)
{
  ...
  // 检查起点和目标点自身是否在碰撞中
  if (getPointColi(ColiPath_start))
  {
    std::cout << "[UGIS] WARNING: start configuration is in collision!" << std::endl;
    return false;
  }
  if (getPointColi(ColiPath_end))
  {
    std::cout << "[UGIS] WARNING: goal configuration is in collision!" << std::endl;
    return false;
  }
  ...
  // Cap the initial maxDis to avoid steer jump across obstacles on long paths
  // if (maxDis > 0.6)
  //   maxDis = 0.6;
  ...
  while(init || maxDis >= maxDis_threshold)
  {
    ...
    for(int i = 0;i < Path_Size - 1;i++)
    {
      ...
      flag = UGISOnce(TEMP_Path, &LocalStart, &LocalEnd, maxDis, flag_CCD, init);
      if(flag == 0)
      {
        break;
      }
...
void UGIS::append_data(bool is_collision, std::vector<double> parent_q, std::vector<double> goal_q, std::vector<double> start_q, std::vector<double> sampled_q)
{
  std::random_device rd;
  std::mt19937 gen(rd());
...
// === 保存为二进制文件 ===
void UGIS::saveToBinary(const Dataset& data, const std::string& filename) {
...
std::vector<Eigen::MatrixXd> UGIS::createOccupancyMapVolume(
...
void UGIS::updateFullTrajectory()
{
  full_trajectory_->updateFromGroupTrajectory(group_trajectory_);
}

void UGIS::setRobotState(std::vector<double>& joint_states)
{
